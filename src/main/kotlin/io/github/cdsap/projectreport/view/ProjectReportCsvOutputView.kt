...
class ProjectReportCsvOutputView(
    ...
    fun writeCsv() {
        ...
        File(csvProject).writeText("""$headers$valuesProject""".trimIndent())
        println("File $csvProject created")

        val csvTasks = "project-report-tasks-${System.currentTimeMillis()}.csv"
        val headersTasks = "user,project,builds,ci,local,mean,p25,p50,p75,p90,p99\n"

        var valuesTasks = ""
        metrics.filter { it.type == Type.Task }.forEach {
            valuesTasks += "${it.value},${it.project},${it.builds},${it.ci},${it.local},${it.mean.inWholeMilliseconds},${it.p25.inWholeMilliseconds},${it.p50.inWholeMilliseconds},${it.p75.inWholeMilliseconds},${it.p90.inWholeMilliseconds},${it.p99.inWholeMilliseconds}\n"
        }
